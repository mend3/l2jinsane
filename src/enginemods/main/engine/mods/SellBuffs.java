    ...
    public void onEvent(Player player, Creature character, String command) {
        ...
        switch (st.nextToken()) {
            ...
            case "buy":
                ...
                sellerName = st.nextToken();
                sellerBuff = World.getInstance().getPlayer(sellerName);
                if (sellerBuff == null)
                    return;
                if (!player.isInsideRadius(sellerBuff, 500, false, false))
                    return;
                ph = PlayerData.get(sellerBuff.getObjectId());
                if (ph == null || !ph.isSellBuff())
                    return;
                if (sellerBuff.getSkillLevel(id) != lvl)
                    return;
                i = ph.getSellBuffPrice();
                if (!player.reduceAdena("sell buff", i, sellerBuff, true))
                    return;
                sellerBuff.addAdena("sell buff", i, player, true);
                ...
                getBuffList(player, sellerBuff, j);
                break;
        }
    }
