...
public class PvpReward extends AbstractMods {
    ...
    private static void giveRewards(Player killer, Player victim) {
        Iterator var2 = ConfigData.PVP_REWARDS.iterator();

        while (var2.hasNext()) {
            RewardHolder reward = (RewardHolder) var2.next();
    ...
    public static class PvPHolder {
        public int _victim;
        public long _time;

        public PvPHolder(final PvpReward param1, int victim, long time) {
            this._victim = victim;
            this._time = time;
        }
    }
