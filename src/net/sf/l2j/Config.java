...
public final class Config {
    ...
    static {
        ...
        CLIENT_PACKET_QUEUE_MAX_PACKETS_PER_SECOND = 160;
        CLIENT_PACKET_QUEUE_MEASURE_INTERVAL = 5;
        CLIENT_PACKET_QUEUE_MAX_AVERAGE_PACKETS_PER_SECOND = 80;
        CLIENT_PACKET_QUEUE_MAX_FLOODS_PER_MIN = 2;
        CLIENT_PACKET_QUEUE_MAX_OVERFLOWS_PER_MIN = 1;
        CLIENT_PACKET_QUEUE_MAX_UNDERFLOWS_PER_MIN = 1;
    ...
    private static final void loadcustomproperties() {
        ...
        ALLOW_DRESS_ME_SYSTEM = customfile.getProperty("AllowDressMeSystem", false);
        ...
        if (LEVEL_REWARDS_ENABLE) {
            ...
            for (String rewards : propert) {
                String[] inforReward = rewards.split(",");
                if (inforReward.length != 2) {
                    System.out.println("[RewardsPerLevel]: invalid config property -> RewardsPerLevel.");
                } else {
                    try {
                        String[] rewardSplit = inforReward[1].split("-");
                        if (rewardSplit.length != 2) {
                            System.out.println("[RewardsPerLevel]: Invalid reward format 57-100 (item-count).");
                        }
    ...
    private static final void loadNpcs() {
        ...
        WAIT_TIME_ANTHARAS = npcs.getProperty("AntharasWaitTime", 30) * '\uea60';
        ...
        WAIT_TIME_FRINTEZZA = npcs.getProperty("FrintezzaWaitTime", 1) * '\uea60';
        ...
        WAIT_TIME_SAILREN = npcs.getProperty("SailrenWaitTime", 5) * '\uea60';
        ...
        WAIT_TIME_VALAKAS = npcs.getProperty("ValakasWaitTime", 30) * '\uea60';
    ...
    private static final void loadPvpEvent() {
        ...
        for (String reward : pvpSpeReward) {
            ...
            if (rewardSplit.length != 2) {
                ...
                try {
                    PVP_SPECIAL_ITEMS_REWARD.add(new int[]{Integer.parseInt(rewardSplit[0]), Integer.parseInt(rewardSplit[1])});
    ...
    public static final class ClassMasterSettings {
        ...
        private void parseConfigLine(String configLine) {
            int job;
            List<IntIntHolder> items;
            for (StringTokenizer st = new StringTokenizer(configLine, ";"); st.hasMoreTokens(); this._rewardItems.put(job, items)) {
