...
public final class Weapon extends Item {
    ...
    public int getMpConsume() {
        if (this._mpConsumeReduceRate > 0 && Rnd.get(100) < this._mpConsumeReduceRate)
            return this._mpConsumeReduceValue;
    ...
    public List<L2Effect> getSkillEffects(Creature caster, Creature target, L2Skill trigger) {
        ...
        if (trigger.isOffensive() != this._skillsOnCast.getSkill().isOffensive())
            return Collections.emptyList();
        if ((trigger.isToggle() || !trigger.isMagic()) && this._skillsOnCast.getSkill().getSkillType() == L2SkillType.BUFF)
            return Collections.emptyList();
