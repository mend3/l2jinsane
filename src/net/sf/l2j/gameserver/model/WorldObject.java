...
public abstract class WorldObject {
    ...
    private final SpawnLocation _position;
    private String _name;
    private int _objectId;
    private NpcTemplate _polyTemplate;
    private PolyType _polyType;
    private int _polyId;
    ...
    public WorldObject(int objectId) {
        ...
        this._position = new SpawnLocation(0, 0, 0, 0);
    ...
    public String toString() {
        return getClass().getSimpleName() + ":" + this.getName() + "[" + this.getObjectId() + "]";
    }
    ...
    public final void spawnMe() {
        ...
        this.setRegion(World.getInstance().getRegion(this._position));
        World.getInstance().addObject(this);
        this.onSpawn();
    }

    public final void spawnMe(Location loc) {
        this.spawnMe(loc.getX(), loc.getY(), loc.getZ());
    }

    public final void spawnMe(Location loc, int heading) {
        this.spawnMe(loc.getX(), loc.getY(), loc.getZ(), heading);
    }

    public final void spawnMe(SpawnLocation loc) {
        this.spawnMe(loc.getX(), loc.getY(), loc.getZ(), loc.getHeading());
    }

    public final void spawnMe(int x, int y, int z) {
        this._position.set(MathUtil.limit(x, WORLD_X_MIN, WORLD_X_MAX), MathUtil.limit(y, WORLD_Y_MIN, WORLD_Y_MAX), z);
        this.spawnMe();
    }

    public final void spawnMe(int x, int y, int z, int heading) {
        this._position.set(MathUtil.limit(x, WORLD_X_MIN, WORLD_X_MAX), MathUtil.limit(y, WORLD_Y_MIN, WORLD_Y_MAX), z, heading);
        this.spawnMe();
    }
    ...
    public void setRegion(WorldRegion newRegion) {
        ...
        for (WorldRegion region : newAreas) {
            if (!oldAreas.contains(region)) {
                ...
                for (WorldObject obj : region.getObjects()) {
                    if (obj != this && this.getInstance().getId() == obj.getInstance().getId()) {
                        obj.addKnownObject(this);
                        this.addKnownObject(obj);
                    }
                }
        ...
        if (region == null) {
            ...
            for (WorldRegion reg : region.getSurroundingRegions()) {
                for (WorldObject obj : reg.getObjects()) {
                    if (obj != this && type.isAssignableFrom(obj.getClass()) && this.getInstance().getId() == obj.getInstance().getId()) {
                        result.add((A) obj);
                    }
                }
            }
    ...
    public final void refreshKnownlist() {
        ...
        if (region != null) {
            for (WorldRegion reg : region.getSurroundingRegions()) {
                for (WorldObject obj : reg.getObjects()) {
                    if (obj != this && !(obj instanceof Door) && !(obj instanceof Fence) && this.getInstance().getId() != obj.getInstance().getId()) {
                        obj.removeKnownObject(this);
                        this.removeKnownObject(obj);
                    }
                }
    ...
    public boolean isSameInstance(WorldObject other) {
        return this.getInstance().getId() == other.getInstance().getId();
    }
}
