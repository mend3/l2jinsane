...
public class Fisherman extends Merchant {
    ...
    public static void showFishSkillList(Player player) {
        List<FishingSkillNode> skills = SkillTreeData.getInstance().getFishingSkillsFor(player);
        if (skills.isEmpty()) {
            int minlevel = SkillTreeData.getInstance().getRequiredLevelForNextFishingSkill(player);
            if (minlevel > 0) {
