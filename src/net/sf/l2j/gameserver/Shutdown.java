...
public class Shutdown extends Thread {
    ...
    public void run() {
        if (this == Shutdown.SingletonHolder.INSTANCE) {
            String var10000 = MODE_TEXT[this._shutdownMode];
            StringUtil.printSection("Under " + var10000 + " process");
            ...
            Runtime.getRuntime().halt(Shutdown.SingletonHolder.INSTANCE._shutdownMode == 2 ? 2 : 0);
        } else {
            this.countdown();
            switch (this._shutdownMode) {
                case 1:
                    Shutdown.SingletonHolder.INSTANCE.setMode(1);
                    Shutdown.SingletonHolder.INSTANCE.run();
                    System.exit(0);
                    break;
                case 2:
                    Shutdown.SingletonHolder.INSTANCE.setMode(2);
                    Shutdown.SingletonHolder.INSTANCE.run();
                    System.exit(2);
            }
        }
            ...
            while (this._secondsShut > 0) {
                if (this._shutdownMode != 3) {
                    switch (this._secondsShut) {
                        ...
                        default:
                            if (this._secondsShut <= 60 && LoginServerThread.getInstance().getServerStatus() != StatusType.DOWN) {
