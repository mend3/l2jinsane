...
public abstract class ArenaTask {
    ...
    public static void spawnNpc2() {
        ...
        try {
            (_npcSpawn2 = new L2Spawn(tmpl)).setLoc(loc2x(), loc2y(), loc2z(), Config.NPC_Heading);
            _npcSpawn2.setRespawnDelay(1);
            SpawnTable.getInstance().addSpawn(_npcSpawn2, false);
            _npcSpawn2.setRespawnState(true);
            _npcSpawn2.doSpawn(false);
            _npcSpawn2.getNpc().getStatus().setCurrentHp(9.99999999E8D);
    ...
    public static int loc2x() {
        int loc2x = Config.NPC_locx;
        return loc2x;
    }

    public static int loc2y() {
        int loc2y = Config.NPC_locy;
        return loc2y;
    }

    public static int loc2z() {
        int loc2z = Config.NPC_locz;
        return loc2z;
    }
