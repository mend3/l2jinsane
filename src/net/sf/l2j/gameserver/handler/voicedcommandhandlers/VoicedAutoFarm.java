...
public class VoicedAutoFarm implements IVoicedCommandHandler {
    ...
    private final String[] VOICED_COMMANDS = new String[]{
            "autofarm", "enableAutoFarm", "radiusAutoFarm", "pageAutoFarm", "enableBuffProtect", "healAutoFarm", "hpAutoFarm", "mpAutoFarm", "enableAntiKs", "enableSummonAttack",
            "summonSkillAutoFarm", "ignoreMonster", "activeMonster"};
    ...
    public boolean useVoicedCommand(String command, Player activeChar, String args) {
        ...
        if (command.startsWith("enableAntiKs")) {
            ...
            if (activeChar.isAntiKsProtected()) {
                ...
                activeChar.sendPacket(new ExShowScreenMessage("Respct Hunt On", 3000, ExShowScreenMessage.SMPOS.TOP_CENTER, false));
            } else {
                activeChar.sendPacket(new SystemMessage(SystemMessageId.DESACTIVATE_RESPECT_HUNT));
                activeChar.sendPacket(new ExShowScreenMessage("Respct Hunt Off", 3000, ExShowScreenMessage.SMPOS.TOP_CENTER, false));
            }
            activeChar.saveAutoFarmSettings();
            showAutoFarm(activeChar);
        }
        if (command.startsWith("enableSummonAttack")) {
            ...
            if (activeChar.isSummonAttack()) {
                ...
                activeChar.sendPacket(new ExShowScreenMessage("Auto Farm Summon Attack On", 3000, ExShowScreenMessage.SMPOS.TOP_CENTER, false));
            } else {
                activeChar.sendPacket(new SystemMessage(SystemMessageId.DESACTIVATE_SUMMON_ACTACK));
                activeChar.sendPacket(new ExShowScreenMessage("Auto Farm Summon Attack Off", 3000, ExShowScreenMessage.SMPOS.TOP_CENTER, false));
            }
            activeChar.saveAutoFarmSettings();
            showAutoFarm(activeChar);
        }
