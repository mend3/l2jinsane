package net.sf.l2j.gameserver.scripting.quests;

import net.sf.l2j.commons.random.Rnd;
import net.sf.l2j.gameserver.model.actor.Creature;
import net.sf.l2j.gameserver.model.actor.Npc;
import net.sf.l2j.gameserver.model.actor.Player;
import net.sf.l2j.gameserver.scripting.Quest;
import net.sf.l2j.gameserver.scripting.QuestState;

public class Q602_ShadowOfLight extends Quest {
    private static final String qn = "Q602_ShadowOfLight";

    private static final int EYE_OF_DARKNESS = 7189;

    private static final int[][] REWARDS = new int[][]{{6699, 40000, 120000, 20000, 20}, {6698, 60000, 110000, 15000, 40}, {6700, 40000, 150000, 10000, 50}, {0, 100000, 140000, 11250, 100}};

    public Q602_ShadowOfLight() {
        super(602, "Shadow of Light");
        setItemsIds(7189);
        addStartNpc(31683);
        addTalkId(31683);
        addKillId(21299, 21304);
    }

    public String onAdvEvent(String event, Npc npc, Player player) {
        String htmltext = event;
        QuestState st = player.getQuestState("Q602_ShadowOfLight");
        if (st == null)
            return htmltext;
        if (event.equalsIgnoreCase("31683-02.htm")) {
            if (player.getLevel() < 68) {
                htmltext = "31683-02a.htm";
            } else {
                st.setState((byte) 1);
                st.set("cond", "1");
                st.playSound("ItemSound.quest_accept");
            }
        } else if (event.equalsIgnoreCase("31683-05.htm")) {
            st.takeItems(7189, -1);
            int random = Rnd.get(100);
            for (int[] element : REWARDS) {
                if (random < element[4]) {
                    st.rewardItems(57, element[1]);
                    if (element[0] != 0)
                        st.giveItems(element[0], 3);
                    st.rewardExpAndSp(element[2], element[3]);
                    break;
                }
            }
            st.playSound("ItemSound.quest_finish");
            st.exitQuest(true);
        }
        return htmltext;
    }

    public String onTalk(Npc npc, Player player) {
        int cond;
        String htmltext = getNoQuestMsg();
        QuestState st = player.getQuestState("Q602_ShadowOfLight");
        if (st == null)
            return htmltext;
        switch (st.getState()) {
            case 0:
                htmltext = "31683-01.htm";
                break;
            case 1:
                cond = st.getInt("cond");
                if (cond == 1) {
                    htmltext = "31683-03.htm";
                    break;
                }
                if (cond == 2)
                    htmltext = "31683-04.htm";
                break;
        }
        return htmltext;
    }

    public String onKill(Npc npc, Creature killer) {
        Player player = killer.getActingPlayer();
        QuestState st = getRandomPartyMember(player, npc, "cond", "1");
        if (st == null)
            return null;
        if (st.dropItems(7189, 1, 100, (npc.getNpcId() == 21299) ? 450000 : 500000))
            st.set("cond", "2");
        return null;
    }
}
