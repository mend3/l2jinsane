...
public class SagasSuperClass extends Quest {
    ...
    public void registerNPCs() {
        addStartNpc(this.NPC[0]);
        addAttackId(this.Mob[2], this.Mob[1]);
        addSkillSeeId(this.Mob[1]);
        addFirstTalkId(this.NPC[4]);
        ...
        int[] questItemIds = this.Items.clone();
        questItemIds[0] = 0;
        questItemIds[2] = 0;
    ...
    public void DeleteSpawn(QuestState st, Npc npc) {
        if (this._SpawnList.containsKey(npc)) {
            this._SpawnList.remove(npc);
            npc.deleteMe();
        }
    }
    ...
    public QuestState findRightState(Npc npc) {
        if (this._SpawnList.containsKey(npc)) {
            Player player = World.getInstance().getPlayer(this._SpawnList.get(npc));
            if (player != null)
    ...
    public void giveHallishaMark(QuestState st2) {
        if (st2.getInt("spawned") == 0)
            if (st2.getQuestItemsCount(this.Items[3]) >= 700) {
                st2.takeItems(this.Items[3], 20);
                Npc Archon = addSpawn(this.Mob[1], st2.getPlayer(), false, 0L, true);
                AddSpawn(st2, Archon);
                st2.set("spawned", "1");
                startQuestTimer("Archon Hellisha has despawned", 600000L, Archon, st2.getPlayer(), false);
                ((Attackable) Archon).addDamageHate(st2.getPlayer(), 0, 99999);
                Archon.getAI().setIntention(IntentionType.ATTACK, st2.getPlayer(), null);
            } else {
                st2.giveItems(this.Items[3], 1);
    ...
    public QuestState findQuest(Player player) {
        ...
        if (st != null && player.getClassId().getId() == QuestClass[this.qnu - 70])
            return st;
        return null;
    }
        ...
        if (st != null) {
            if (event.equalsIgnoreCase("0-011.htm") || event.equalsIgnoreCase("0-012.htm") || event.equalsIgnoreCase("0-013.htm") || event.equalsIgnoreCase("0-014.htm") || event.equalsIgnoreCase("0-015.htm")) {
                ...
                if (player.getLevel() >= 76) {
                    ...
                    st.set("cond", "0");
                    htmltext = "0-07.htm";
                    st.takeItems(this.Items[10], -1);
                    st.rewardExpAndSp(2299404L, 0);
                ...
                st.set("cond", "4");
                st.takeItems(this.Items[0], 1);
                if (this.Items[11] != 0)
                    st.takeItems(this.Items[11], 1);
                st.giveItems(this.Items[1], 1);
                htmltext = "1-06.htm";
                ...
                st.set("cond", "5");
                st.takeItems(this.Items[1], 1);
                st.giveItems(this.Items[4], 1);
                htmltext = "2-06.htm";
                ...
                st.set("cond", "13");
                st.takeItems(this.Items[2], 1);
                st.giveItems(this.Items[7], 1);
                htmltext = "3-08.htm";
            } else if (event.equalsIgnoreCase("4-1")) {
                htmltext = "4-010.htm";
            } else if (event.equalsIgnoreCase("4-2")) {
                st.giveItems(this.Items[9], 1);
                st.set("cond", "18");
                st.playSound("ItemSound.quest_middle");
                htmltext = "4-011.htm";
            } else {
                ...
                if (event.equalsIgnoreCase("5-1")) {
                    ...
                    if (st.getInt("spawned") == 1) {
                        ...
                        Npc Mob_1 = addSpawn(this.Mob[0], this.X[0], this.Y[0], this.Z[0], 0, false, 0L, true);
                        st.set("spawned", "1");
                        startQuestTimer("Mob_1 Timer 1", 500L, Mob_1, player, false);
                        startQuestTimer("Mob_1 has despawned", 300000L, Mob_1, player, false);
                        AddSpawn(st, Mob_1);
                        htmltext = "7-02.htm";
                    } else {
                        htmltext = "7-04.htm";
                    ...
                    st.set("cond", "10");
                    st.takeItems(this.Items[6], 1);
                    cast(npc, player, 4546, 1);
                    st.playSound("ItemSound.quest_middle");
                    htmltext = "7-06.htm";
                } else if (event.equalsIgnoreCase("8-1")) {
                    st.set("cond", "14");
                    st.takeItems(this.Items[7], 1);
                    cast(npc, player, 4546, 1);
                    st.playSound("ItemSound.quest_middle");
                    htmltext = "8-02.htm";
                } else if (event.equalsIgnoreCase("9-1")) {
                    st.set("cond", "17");
                    st.takeItems(this.Items[8], 1);
                    cast(npc, player, 4546, 1);
                    st.playSound("ItemSound.quest_middle");
                    htmltext = "9-03.htm";
                } else if (event.equalsIgnoreCase("10-1")) {
                    if (st.getInt("Quest0") == 0) {
                        Npc Mob_3 = addSpawn(this.Mob[2], this.X[1], this.Y[1], this.Z[1], 0, false, 0L, true);
                        Npc Mob_2 = addSpawn(this.NPC[4], this.X[2], this.Y[2], this.Z[2], 0, false, 0L, true);
                        AddSpawn(st, Mob_3);
                        AddSpawn(st, Mob_2);
                        st.set("Mob_2", String.valueOf(Mob_2.getObjectId()));
                        st.set("Quest0", "1");
                        st.set("Quest1", "45");
                        startQuestTimer("Mob_3 Timer 1", 500L, Mob_3, player, false);
                        startQuestTimer("Mob_2 Timer 1", 500L, Mob_2, player, false);
                        startQuestTimer("Mob_3 has despawned", 59000L, Mob_3, player, false);
                        startQuestTimer("Mob_2 has despawned", 60000L, Mob_2, player, false);
                        htmltext = "10-02.htm";
                    ...
                    if (event.equalsIgnoreCase("Mob_1 Timer 1")) {
                    ...
                    if (event.equalsIgnoreCase("Mob_3 Timer 1")) {
                        Npc Mob_2 = (Npc) World.getInstance().getObject(st.getInt("Mob_2"));
                        if (Mob_2 == null)
                            return null;
                        if (this._SpawnList.containsKey(Mob_2) && this._SpawnList.get(Mob_2) == player.getObjectId()) {
                    ...
                    if (event.equalsIgnoreCase("Mob_3 has despawned")) {
                        npc.broadcastNpcSay(Text[15]);
                        st.set("Quest0", "2");
                        DeleteSpawn(st, npc);
                        return null;
                    }
                    if (event.equalsIgnoreCase("Mob_2 Timer 1")) {
                        npc.broadcastNpcSay(Text[7]);
                        startQuestTimer("Mob_2 Timer 2", 1500L, npc, player, false);
                        if (st.getInt("Quest1") == 45)
                            st.set("Quest1", "0");
                        return null;
                    }
                    if (event.equalsIgnoreCase("Mob_2 Timer 2")) {
                        npc.broadcastNpcSay(Text[8].replace("PLAYERNAME", player.getName()));
                        startQuestTimer("Mob_2 Timer 3", 10000L, npc, player, false);
                        return null;
                    }
                    if (event.equalsIgnoreCase("Mob_2 Timer 3")) {
                        if (st.getInt("Quest0") == 0) {
                            startQuestTimer("Mob_2 Timer 3", 13000L, npc, player, false);
                            if (Rnd.get(2) == 0) {
                    ...
                    if (event.equalsIgnoreCase("Mob_2 has despawned")) {
                        ...
                        if (st.getInt("Quest0") == 1 || st.getInt("Quest0") == 2 || st.getInt("Quest1") > 3) {
                            ...
                            startQuestTimer("Mob_2 has despawned", 1000L, npc, player, false);
                        }
                        return null;
                    }
                }
    ...
    public String onTalk(Npc npc, Player player) {
        ...
        if (st != null) {
            ...
            if (st.isCompleted() && npcId == this.NPC[0]) {
                ...
                if (cond == 0) {
                    ...
                    if (npcId == this.NPC[1] && st.hasQuestItems(this.Items[0])) {
                        htmltext = "1-02.htm";
                        if (this.Items[11] == 0 || st.hasQuestItems(this.Items[11]))
                            htmltext = "1-03.htm";
                    }
                } else if (cond == 4) {
                    if (npcId == this.NPC[1]) {
                    ...
                    if (npcId == this.NPC[3])
                        if (st.hasQuestItems(this.Items[2])) {
                            htmltext = "3-05.htm";
                        } else {
                            htmltext = "3-04.htm";
                        }
    ...
    public String onSkillSee(Npc npc, Player player, L2Skill skill, WorldObject[] targets, boolean isPet) {
        if (this._SpawnList.containsKey(npc) && this._SpawnList.get(npc) != player.getObjectId()) {
            Player quest_player = World.getInstance().getPlayer(this._SpawnList.get(npc));
            ...
            if (npcId == element) {
                ...
                if (st1 != null)
                    if (st1.getInt("cond") == 15) {
                        ...
                        st1.giveItems(this.Items[8], 1);
                        st1.takeItems(this.Items[3], -1);
                        st1.set("cond", "16");
        ...
        if (st != null && npcId != this.Mob[2]) {
            ...
            if (npcId == this.Mob[0] && cond == 8) {
                ...
                if (!player.isInParty())
                    if (st == st2) {
                        npc.broadcastNpcSay(Text[4]);
                        st.giveItems(this.Items[8], 1);
                        st.takeItems(this.Items[3], -1);
                        st.set("cond", "16");
                        st.playSound("ItemSound.quest_middle");
                    } else {
                        npc.broadcastNpcSay(Text[5]);
