...
public class FourSepulchers extends L2AttackableAIScript {
    ...
    protected void registerNpcs() {
        ...
        addKillId(18120, 18121, 18122, 18123, 18124, 18125, 18126, 18127, 18128, 18129,
                18130, 18131, 18149, 18158, 18159, 18160, 18161, 18162, 18163, 18164,
                18165, 18183, 18184, 18212, 18213, 18214, 18215, 18216, 18217, 18218,
                18219, 18150, 18151, 18152, 18153, 18154, 18155, 18156, 18157, 18141,
                18142, 18143, 18144, 18145, 18146, 18147, 18148, 18220, 18221, 18222,
                18223, 18224, 18225, 18226, 18227, 18228, 18229, 18230, 18231, 18232,
                18233, 18234, 18235, 18236, 18237, 18238, 18239, 18240, 25339, 25342,
                25346, 25349);
        addSpawnId(18150, 18151, 18152, 18153, 18154, 18155, 18156, 18157, 18231, 18232,
                18233, 18234, 18235, 18236, 18237, 18238, 18239, 18240, 18241, 18242,
                18243, 25339, 25342, 25346, 25349);
    }
                ...
                if (player != null) {
                    ...
                    if (party != null) {
                        for (Player member : party.getMembers()) {
                            ...
                            if (qs != null && (qs.isStarted() || qs.isCompleted()) && member.getInventory().getItemByItemId(7262) == null && member.isInsideRadius(npc, Config.PARTY_RANGE, true, false))
                                member.addItem("Quest", cupId, 1, member, true);
                        }
                    } else {
                        QuestState qs = player.getQuestState("Q620_FourGoblets");
                        if (qs != null && (qs.isStarted() || qs.isCompleted()) && player.getInventory().getItemByItemId(7262) == null && player.isInsideRadius(npc, Config.PARTY_RANGE, true, false))
    ...
    public String onSpawn(Npc npc) {
        switch (npc.getNpcId()) {
            ...
            case 18156:
            case 18157:
                startQuestTimer("safety", 300000L, npc, null, false);
                startQuestTimer("aggro", 1000L, npc, null, false);
                break;
            case 18231:
            case 18232:
            case 18233:
