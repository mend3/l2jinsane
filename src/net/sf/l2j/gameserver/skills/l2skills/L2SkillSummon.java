...
public class L2SkillSummon extends L2Skill {
    ...
    public boolean checkCondition(Creature activeChar) {
        if (activeChar instanceof Player player) {
            if (this.isCubic()) {
                if (this.getTargetType() != SkillTargetType.TARGET_SELF) {
                    return true;
                }
            ...
            if (this._npcId == 0) {
                ...
                if (activeChar.getSummon() == null && !activeChar.isMounted()) {
                    ...
                    if (summonTemplate == null) {
                        int var21 = this._npcId;
                        _log.warning("Summon attempt for nonexisting NPC ID: " + var21 + ", skill ID: " + this.getId());
                    } else {
                        Servitor summon;
                ...
                if (_cubicSkillLevel > 100) {
                    _cubicSkillLevel = Math.round((float) ((this.getLevel() - 100) / 7 + 8));
                }
                ...
                if (targets.length <= 1) {
                    ...
                    if (activeChar.getCubics().size() > activeChar.getSkillLevel(143)) {
                        ...
                        activeChar.addCubic(this._npcId, _cubicSkillLevel, this.getPower(), this._activationtime, this._activationchance, this._summonTotalLifeTime, false);
                        ...
                        if (obj instanceof Player) {
                            ...
                            if (player.getCubics().size() <= mastery) {
                                if (player == activeChar) {
                                    player.addCubic(this._npcId, _cubicSkillLevel, this.getPower(), this._activationtime, this._activationchance, this._summonTotalLifeTime, false);
                                } else {
                                    player.addCubic(this._npcId, _cubicSkillLevel, this.getPower(), this._activationtime, this._activationchance, this._summonTotalLifeTime, true);
                                }
