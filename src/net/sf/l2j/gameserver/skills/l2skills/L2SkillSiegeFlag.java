    ...
    public static boolean checkIfOkToPlaceFlag(Player player, boolean isCheckOnly) {
        ...
        if (siege != null && siege.checkSide(player.getClan(), SiegeSide.ATTACKER)) {
            if (!player.isClanLeader()) {
                ...
                if (player.getKnownTypeInRadius(SiegeFlag.class, 400).isEmpty()) {
                    return true;
                }
    ...
    public void useSkill(Creature activeChar, WorldObject[] targets) {
        if (activeChar instanceof Player) {
            ...
            if (checkIfOkToPlaceFlag(player, true)) {
                StatSet npcDat = makeNpcDat(player);
                SiegeFlag flag = new SiegeFlag(player, IdFactory.getInstance().getNextId(), new NpcTemplate(npcDat));
