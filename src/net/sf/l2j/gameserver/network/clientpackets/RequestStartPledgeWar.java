...
public final class RequestStartPledgeWar extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (player == null)
            return;
        Clan attackerClan = player.getClan();
        ...
        Clan attackedClan = ClanTable.getInstance().getClanByName(this._pledgeName);
        ...
        if (!attackerClan.getAttackerList().contains(Integer.valueOf(attackedClan.getClanId())) && (attackedClan.getLevel() < 3 || attackedClan.getMembersCount() < Config.ALT_CLAN_MEMBERS_FOR_WAR)) {
        ...
        if (attackedClan.getDissolvingExpiryTime() > 0L) {
