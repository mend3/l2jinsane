    ...
    protected void runImpl() {
        if (this.getClient() != null) {
            ...
            if (player == null) {
                ...
                if (player.getCurrentHp() < (double) 0.5F && player.isMortal()) {
                ...
                if (player.isSubClassActive() && player.getSubClasses().get(player.getClassIndex()).getLevel() > Config.SUBCLASS_MAX_LEVEL - 1) {
                    player.getSubClasses().get(player.getClassIndex()).setLevel((byte) (Config.SUBCLASS_MAX_LEVEL - 1));
                }

                player.checkEquipmentXPvps();
                Hwid.enterlog(player, this.getClient());
                ...
                this.sendPacket(new SkillCoolTime(player));
