...
public class Pagination<A> extends AbstractList<A> {
    ...
    public Pagination(Stream<A> stream, int page, int limit, Predicate<A> filter, Comparator<A> comparator) {
        this._list = initList(stream, filter, comparator);
        if (page <= 0 || limit <= 0 || this._list.isEmpty())
            return;
        this._limit = limit;
        this._total = this._list.size() / limit + ((this._list.size() % limit == 0) ? 0 : 1);
        this._page = MathUtil.limit(page, 1, this._total);
        this._list = this._list.subList((Math.min(page, this._total) - 1) * limit, Math.min(Math.min(page, this._total) * limit, this._list.size()));
    }
    ...
    public void generateSpace(StringBuilder sb) {
        IntStream.range(size(), this._limit).forEach(x -> sb.append("<img height=17>"));
    }

    public void generateSpace(StringBuilder sb, String content) {
        IntStream.range(size(), this._limit).forEach(x -> sb.append(content));
    }
    ...
    public void generatePages(StringBuilder sb, String action) {
        ...
        int index;
        for (index = this._page - 5; index < this._page - 1; index++) {
            StringUtil.append(sb, "<td width=30>" + ((index < 0) ? "" : ("<a action=\"" + action.replace("%page%", String.valueOf(index + 1)) + "\">" + ((index + 1 < 10) ? "0" : "") + index + 1 + "</a>")) + "</td>");
        }
        StringUtil.append(sb, "<td width=30><font color=LEVEL>", ((this._page < 10) ? "0" : "") + ((this._page < 10) ? "0" : ""), "</font></td>");
        for (index = this._page; index < this._page + 4; index++) {
            StringUtil.append(sb, "<td width=30>" + ((index < this._total) ? ("<a action=\"" + action.replace("%page%", String.valueOf(index + 1)) + "\">" + ((index + 1 < 10) ? "0" : "") + index + 1 + "</a>") : "") + "</td>");
        }
        StringUtil.append(sb, "<td width=30 align=center><button action=\"", action.replace("%page%", String.valueOf(this._total)), "\" back=L2UI_CH3.next1_down fore=L2UI_CH3.next1 width=16 height=16></td></tr></table>");
    }
